...
### 주차별 문서 작성 규칙 (`docs/week-XX-*.md`)

주차 작업 완료 시 해당 주차 문서를 **실제 구현 내용 기준**으로 작성한다. 템플릿(`_template-week.md`)의 예상 코드는 실제 코드로 교체한다.

#### 필수 포함 섹션

| 섹션 | 내용 |
|------|------|
| **구현 상세** | 변경/신규 파일별로 실제 코드 스니펫 + 설계 의도 설명. 코드만 붙여넣지 말고 **왜 이렇게 구현했는지** 서술 |
| **데이터 흐름 추적** | 대표 입력 데이터가 각 Step을 거치며 어떻게 처리되는지 단계별 추적 (ASCII 다이어그램 권장) |
| **테스트 데이터** | 테스트 입력 파일의 각 레코드별 처리 결과를 표로 정리 |
| **테스트 시나리오** | 시나리오별 입력/기대값/검증 포인트를 표로 정리 |
| **트러블슈팅 로그** | 현상 → 원인 → 해결 형식. 실제 겪은 이슈만 기록 |
| **변경된 파일** | 수정/신규 파일 목록과 각 파일의 변경 내용 요약 |

#### 작성 원칙

- **코드 스니펫은 실제 구현 기준**: 템플릿의 예상 코드나 의사 코드를 남기지 않는다
- **설계 판단 근거 포함**: "이렇게 했다"가 아니라 "A 방식은 ~문제가 있어서 B 방식을 선택했다" 형태
- **전후 비교**: 이전 주차 대비 변경점이 있으면 before/after 비교 표 포함
- **빈약한 문서 금지**: 코드를 붙여넣기만 한 문서는 안 됨. 각 구현에 대한 설명, 흐름 추적, 발견한 동작 특성을 충분히 서술
